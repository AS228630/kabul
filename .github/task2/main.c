...
typedef struct ThinTest {//структура для теста
    int low;//низкий
    int high;//высокий
} ThinTest;

void *remove_test(void *data) {//тест по удалению
    ThinTest *test = (ThinTest *) data;

    for (int i = test->low; i < test->high; i += 3)
        myRemove(list, i);//удаляем узлы
}

void *create_arr() {//создать массив
    for (int i = 0; i < 100; i += 3)
        insert(list, i, i);//вставляем значения
}
...
void *insert_test_reverse(void *data) {//вставляем в обратном порядке
    ThinTest *test = (ThinTest *) data;
...
void test() {
    pthread_t create_thread;//создаем поток
    pthread_create(&create_thread, NULL, create_arr, NULL);
    pthread_join(create_thread, NULL);//подсоединяем поток
