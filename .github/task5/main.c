...
#define THREADS_COUNT 1//количество потоков
#define MAX 2000000
int temp[MAX];
...
mutex bounds_mutex;
mutex temp_mutex;//мьютексы

typedef struct Interval {//интервал
    int low;//наименьший
    int high;//наибольший
} Interval;

void merge(int a[], int low, int mid, int high) {//метод бинарного деления
    int left[mid - low + 1];
    int right[high - mid];
...
void merge_sort(int a[], int low, int high) {
    ...
    if (low < high) {
        merge_sort(a, low, mid);//сортировка
...
Interval init_bounds() {//инициализация рамок
    int low, high;
